...
def parse_blocks_from_text(text: str):
    ...
    for b in blocks:
        ...
        for mp in RE_PHASE.finditer(b):
            ...
            zc_val = float(mp.group("zc"))
            zc_unit = mp.group("zc_unit").lower()
            if zc_unit == "mm":
                zc_val *= 1e-3
            key = "push" if ph.startswith("push") else "retract"
            d[key] = (float(mp.group("m")), zc_val, float(mp.group("th")))out.append(d)
    return out
...
def build_summary_from_txt(txt_dir: str, strict_two_pushes: bool):
    ...
    if per_file_phase_rows:
        ...
        aggph = dph.groupby(["object", "n_safety", "push_idx", "phase"]).agg(
            n_files=("source_txt", "count"),
            phase_coverage=("has_phase", "mean"),  # fraction of files where this phase was present
        ...
        summary_long = pd.DataFrame(columns=[
            "object","n_safety","push_idx","phase","n_files","phase_coverage",
...
def make_main_figure(summary: pd.DataFrame, out_png: str, title_suffix: str = ""):
    ...
    for i, obj in enumerate(OBJECT_ORDER):
        ...
        ax.bar(x - width, sub["m_err_pct"].to_numpy(float), width=width, label="Mass (%GT)")
        ax.bar(x,         sub["zc_err_pct"].to_numpy(float), width=width, label="$z_c$ (%GT)")
        ax.bar(x + width, sub["theta_star_err_pct"].to_numpy(float), width=width, label="θ* (%GT)")
    ...
    axes[0].legend(handles, labels, loc="upper left", fontsize=14, frameon=True, framealpha=0.9)
...
def make_push_compare_figure(summary: pd.DataFrame, out_png: str, title_suffix: str = ""):
    ...
    for r, obj in enumerate(OBJECT_ORDER):
        ...
        for c, (title, p1col, p2col) in enumerate(metrics):
            ...
            ax.bar(x - bar_w/2, sub[p1col].to_numpy(float), width=bar_w, label="Push1")
            ax.bar(x + bar_w/2, sub[p2col].to_numpy(float), width=bar_w, label="Push2")
