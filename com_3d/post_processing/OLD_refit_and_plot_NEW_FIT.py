...
class RefitEstimator:
    ...
    def _estimate_params(self, th, tau_flat, tag="phase"):
        ...
        tau_vec = np.asarray(tau_flat, float).reshape(-1, 3)
        tau_proj = tau_vec @ self.e_hat
        ...
        tau_fit_vec = _fit_target(th_in, m_est, zc_est).reshape(-1, 3)
        resid_nl = (tau_in - tau_fit_vec).reshape(-1)
    ...
    def process(self, csv_path, q_ref=None):
        """
        Load CSV, process, fit push and retract phases.
...
def make_fit_plot(process_result, theta_star_gt_deg, csv_path, decimate=1):
    ...
    # We'll use e_hat from the estimator (but it's not in the result, so approximate)
    # For now, assume rotation is around y-axis [0, 1, 0]
    e_hat = np.array([0, 1, 0])
    ...
    rc0 = np.array([-0.05, 0, 0])
    d = np.linalg.norm(rc0 - np.dot(rc0, e_hat) * e_hat)
