...
class ForceWatcher:
    ...
    def ft_cb(self, msg):
        ...
        self.debug_msg(f"New Peak!: {self.peak:.3f} N", 0.25)
        ...
        if self.state == "baseline":
            ...
            if not self.baseline_ready:
                ...
                if self.baseline_buf.count(0.0) == 0:
                    self.debug_msg("Baseline buffer filled. No longer updating baseline noise")
            ...
            if (f_med-self.noise_floor) > self.contact_delta:
                ...
                self.debug_msg(f"Contact magnitude NOT met: \n \
                               f_med={f_med:.3f} N <= noise_floor+contact_delta={self.noise_floor+self.contact_delta:.3f} N\n \
