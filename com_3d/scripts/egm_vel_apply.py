...
# TODO TODO TODO TODO: ============================ FIX THIS FILE ============================
...
def main():
    ...
    rospy.loginfo("Starting EGM w/ settings and JGVC init...")

    # Before doing ANYTHING, wait for EGM jtc to be available
    # rospy.wait_for_service("/egm/"+JGVC_name+"query_state")
    # rospy.wait_for_service("/egm/joint_group_velocity_controller/query_state")
    ...
    rospy.loginfo("Starting %s via controller manager...", JGVC_name)
    ...
    for attempt in range(1,6):
        try:
            switch(start_controllers=[JGVC_name], 
                stop_controllers=[], 
                strictness=1,
                start_asap=False, 
                timeout=0.0)
            rospy.loginfo("JGVC started")
            break
        ...
        rospy.logerr("Failed to start JGVC after several attempts, aborting.")
        return


    rospy.loginfo("Init complete: EGM running, JGVC started. Ready to receive trajectories.")
    rospy.spin()
